...
public function index()
{
    try {
        ...
        $testList = TestModel::orderBy('id', 'desc')->paginate(5); //order by & pagination paginate(5)แสดงรายการเริ่ทต้น 5 รายการ
        return view('test.list', compact('testList'));
        //return view('bootstrap-design.index');
    } catch (\Exception $e) {
       // \Log::error('Admin list error: '.$e->getMessage());
         return view('errors.404'); //โฟลเดอร์ error ไฟล์ชื่อ 404
    }
}
 ...
 public function edit($id)
    {
        try {
            ...
            if (isset($test)) {
                $id = $test->id;
                $name = $test->name;
                return view('test.edit', compact('id', 'name'));
            }
        } catch (\Exception $e) {
            //return response()->json(['error' => $e->getMessage()], 500); //สำหรับ debug
            return view('errors.404');
